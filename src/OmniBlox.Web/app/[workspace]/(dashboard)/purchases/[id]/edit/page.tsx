...
const normalizeError = (error: unknown): string => {
  ...
  return "Something went wrong while saving the purchase order.";
};
...
export default function EditPurchasePage() {
  ...
  const { getById, update } = usePurchasesApi();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        setLoadError(null);
        const result = await getById(purchaseId);
        if (active) {
          setPurchase(result);
          setFormData({
            supplierId: result.supplier?.id ?? "",
            supplierName: result.supplier?.name ?? "",
            orderDate: result.orderDate?.split("T")[0] ?? today,
            warehouseId: result.warehouse?.id ?? result.warehouseId ?? "",
            notes: "",
          });
          setItems(
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!purchase) {
      setSubmitError("Purchase order not found.");
      return;
    }
    ...
    if (items.length === 0) {
      setSubmitError("Add at least one product to update the purchase order.");
      return;
    }
    ...
    try {
      ...
      await update(purchase.id, payload);
  ...
  if (!purchaseId) {
    return <div className="p-6">Purchase identifier is missing.</div>;
  }
    ...
    return (
      <div className="space-y-5 p-6">
        <div className="flex items-center gap-1 text-sm text-muted-foreground mb-0.5">
          ...
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-foreground">Edit Purchase</span>
        </div>
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-[18px] font-bold text-foreground">Edit Purchase</h1>
      ...
      <form id="edit-purchase-form" onSubmit={handleSubmit} className="grid gap-5 lg:grid-cols-[1fr_320px]">
        {/* Left Column */}
        <div className="space-y-5">
          <Card className="border rounded-[5px] bg-card shadow-sm">
            <CardHeader className="px-5 py-[15px] border-b">
              ...
              <CardDescription className="text-xs">Update supplier and purchase order details</CardDescription>
            </CardHeader>
