...
export default function PurchaseReturnDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const data = await getPurchaseReturn(id);
        if (!mounted) return;
        setPr(data);
      } catch (e: any) {
        if (!mounted) return;
  ...
  const handleStatusChange = async (newStatus: "PENDING" | "PROCESSING" | "COMPLETED" | "CANCELLED") => {
    if (!pr || statusUpdateLoading) return;
    try {
      setStatusUpdateLoading(true);
      const updated = await updatePurchaseReturn(pr.id, { status: newStatus });
      setPr(updated);
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-1 text-sm text-muted-foreground mb-0.5">
          ...
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-foreground">Purchase Return Detail</span>
        </div>
        <div className="border rounded-[5px] bg-card shadow-sm py-12 text-center text-muted-foreground">
          <Package className="h-12 w-12 mx-auto mb-3 text-muted-foreground/50" />
          <p className="font-medium">{error || "Purchase return not found"}</p>
        </div>
      </div>
    );
  }

  const curStatus = pr.status;
  const statusLabel = statusConfig[curStatus]?.label || pr.status;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-1 text-sm text-muted-foreground mb-0.5">
        ...
        <Link href="/purchase-returns" className="hover:text-foreground transition-colors">Purchase Returns</Link>
        <ChevronRight className="h-3.5 w-3.5" />
        <span className="text-foreground">{pr.referenceNumber}</span>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-[18px] font-bold text-foreground">{pr.referenceNumber}</h1>
      ...
      {pr.status !== "COMPLETED" && (
        <div className="flex items-center gap-2 px-5 py-3 bg-amber-50 text-amber-800 text-sm border border-amber-200 rounded-[5px]">
          ...
          <span>
            ...
            {pr.purchaseOrderId && (pr.status === "PENDING" || pr.status === "PROCESSING") && (
              <span> The original purchase order will not show return indicators until you mark this return as <strong>completed</strong>.</span>
            )}
          </span>
        </div>
      )}
      {pr.status === "COMPLETED" && pr.purchaseOrderId && (
        <div className="flex items-center gap-2 px-5 py-3 bg-emerald-50 text-emerald-800 text-sm border border-emerald-200 rounded-[5px]">
          ...
          <span>This return has been processed. The original purchase order now shows return indicators with the returned quantities.</span>
        </div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="border rounded-[5px] bg-card shadow-sm p-5">
          ...
          <p className="text-lg font-semibold">{new Date(pr.returnDate).toLocaleDateString("en-US", { day: "2-digit", month: "short", year: "numeric" })}</p>
        </div>
        <div className="border rounded-[5px] bg-card shadow-sm p-5">
          ...
          <div className="p-5">
            ...
            <div className="flex justify-end pt-4">
              <div className="flex justify-between items-center w-[220px]">
                ...
                <span className="text-xl font-bold tabular-nums">{formatCurrency.format(Number(pr.totalAmount))}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. Return {pr.referenceNumber} will be removed permanently.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
